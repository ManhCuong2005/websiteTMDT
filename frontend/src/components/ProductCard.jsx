  ...
  return (
    <article className="product-card">
      ...
      <div className="product-card-body">
        ...
        <div className="rating-line"><span>★</span> {Number(product.averageRating || 0).toFixed(1)} <small>({product.reviewCount || 0})</small></div>
        <div className="price-row">
          <div><strong>{money(product.price)}</strong>{product.compareAtPrice && <del>{money(product.compareAtPrice)}</del>}</div>
          <button className="icon-button primary" onClick={add} disabled={product.stockQuantity <= 0} aria-label="Thêm vào giỏ"><Icon name="cart"/></button>
        </div>
      </div>
    </article>
  )
